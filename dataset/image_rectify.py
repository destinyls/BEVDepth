...
class ImageRectify(object):
    def __init__(self, image_shape, roll_range=[0.0, 1.0], pitch_range=[0.0, 1.0], ratio=[1.0, 0.05]):
    ...
    def __call__(self, image, lidar2cam, cam_intrinsic, is_roll=False, is_pitch=False, is_intrin=False):
        ...
        if is_intrin:
            cam_intrinsic_rectify, _ = self.rectify_cam_intrinsic(cam_intrinsic)
        if is_roll:
            lidar2cam_rectify, _ = self.rectify_roll_params(lidar2cam_rectify)
        if is_pitch:
            lidar2cam_rectify = self.rectify_pitch_params(lidar2cam_rectify)
