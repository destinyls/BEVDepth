...
class LSSFPN(nn.Module):
    ...
    def homo_warping(self, depth, feature, sensor2ego_mat, intrin_mat, ida_mat, geom_xyz):     
        ...
        with torch.no_grad():
            ...
            mask = (proj_xy[:,:,0,:] >= 0) & (proj_xy[:,:,0,:] < width) & (proj_xy[:,:,1,:] >= 0) & (proj_xy[:,:,1,:] < height)
            mask = mask.view(batch * num_cams, 1, height, width)
        ...
        warped_ref_depth = F.grid_sample(ref_depth.view(batch * num_cams, channel_depth, height, width),
                                         grid.view(batch * num_cams, height, width, 2), mode='bilinear', padding_mode='zeros')
        warped_ref_fea = torch.where(mask.repeat(1, channel_feature, 1, 1), warped_ref_fea, src_feature.view(-1, channel_feature, height, width))
        warped_ref_depth = torch.where(mask.repeat(1, channel_depth, 1, 1), warped_ref_depth, src_depth.view(-1, channel_depth, height, width))
        ...
        src_geom_xyz = src_geom_xyz.view(bs, num_cams, src_geom_xyz.shape[3], 
